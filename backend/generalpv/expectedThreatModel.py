...
from backend.generalpv.basemodel import BaseModel
from backend.generalpv.preprocessor import DatasetPreprocessor
...
class ExpectedThreatModel(BaseModel):
    ...
    def calculate_expected_threat(self, **kwargs):
        ...
        features = ['dist_to_goal', 'type_Carry',
                    'type_Pass', 'type_Shot'] + player_feats
    ...
    def load_and_prep(self):
        ...
        opp_count = len([c for c in df.columns if c.endswith(
            '_dist') and c.startswith('opp_')])
        ...
        tm_count = len([c for c in df.columns if c.endswith(
            '_dist') and c.startswith('tm_')])
