...
def calculate_shooting_lane_blocked(
    ...
    for i in range(n_segments):
        target_y = goal_bottom + (goal_top - goal_bottom) * (i + 0.5) / n_segments
        ...
        for opp in opponents:
            ...
            t = max(0, min(1, ((opp['x'] - ball_x) * dx + (opp['y'] - ball_y) * dy) / (line_len ** 2)))
...
def calculate_passing_options(
    ...
    for tm in teammates:
        ...
        if not lane_blocked:
            ...
            position_gain = (tm['x'] - ball_x) / PITCH_LENGTH  # Forward progress
            clearance = min(min_defender_dist / 10.0, 1.0)  # How clear the lane is
...
def extract_ball_features(row: pd.Series) -> np.ndarray:
    ...
    features = np.array([
        ...
        ball_x / PITCH_LENGTH,         # Progress (same as norm_x but semantically different)
        abs(ball_y - GOAL_Y) / (PITCH_WIDTH / 2),  # Centrality (0 = center, 1 = edge)
...
def extract_global_features(
    ...
    shooting_lane_blocked = calculate_shooting_lane_blocked(ball_x, ball_y, opponents)
    ...
    viable_passes, best_lane_quality = calculate_passing_options(ball_x, ball_y, teammates, opponents)
    
    # NEW: Space control
    space_control = calculate_space_control(ball_x, ball_y, teammates, opponents)
    ...
    final_third_advantage = (teammates_final_third - opponents_final_third) / 6.0  # Normalize
    ...
    counter_attack = 1.0 if (n_opponents < 6 and ball_x > 60 and nearest_defender_dist > 5) else 0.0
    
    features = np.array([
        # Original features
        n_teammates / 11.0,                          # 0: Normalized teammate count
        n_opponents / 11.0,                          # 1: Normalized opponent count
        min(defensive_pressure, 5.0) / 5.0,          # 2: Capped and normalized pressure
        nearest_defender_dist / PITCH_LENGTH,        # 3: Normalized nearest defender
        teammates_in_box / 5.0,                      # 4: Normalized (max ~5 in box)
        opponents_in_box / 6.0,                      # 5: Normalized (max ~6 in box)
        shooting_lane_blocked,                       # 6: Already [0, 1]
        nearest_teammate_dist / PITCH_LENGTH,        # 7: Normalized nearest teammate
        1.0 if n_opponents == 0 else 0.0,            # 8: No opponents visible (rare)
        1.0 if nearest_defender_dist < 3.0 else 0.0, # 9: Under immediate pressure
        # New features
        min(viable_passes, 5) / 5.0,                 # 10: Viable passing options
...
def process_row(row: pd.Series) -> Optional[Dict]:
    ...
    global_features = extract_global_features(ball_x, ball_y, teammates, opponents)
...
def preprocess_dataset(
    ...
    if verbose:
        ...
        print(f"  Teammates per sample: mean={np.mean(tm_counts):.1f}, max={max(tm_counts)}")
        print(f"  Opponents per sample: mean={np.mean(opp_counts):.1f}, max={max(opp_counts)}")
...
def process_inference_input(
    ...
    all_teammates = [{'x': p['x'], 'y': p['y'], 'is_keeper': False} for p in teammates]
    all_opponents = [{'x': p['x'], 'y': p['y'], 'is_keeper': False} for p in opponents]
    ...
    # Global features
    global_features = extract_global_features(ball_x, ball_y, all_teammates, all_opponents)
